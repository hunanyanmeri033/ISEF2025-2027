...
SYS_SEED = 42    # Seed for random initial weights
SYS_BATCH_SIZE =  100
SYS_LEARNING_RATE = 0.03
SYS_LAYERS = 4
n_qubits = 6
EPOCHS_SETTING = 100
...
class BreastCancerDataset(Dataset):
    def __init__(self, train=False, validation=False, test=False, seed=SEED):
        ...
        X_val = np.load("../data/processed/X_valq.npy")
        y_val = np.load("../data/processed/y_valq_tf.npy")
...
num_classes = 2
print(breast_cancer_train[0][0].shape)
input_dimension = int(breast_cancer_train[0][0].shape[0])
...
def variational_circuit_angle_embedding(input, weights, out):
    ...
    return qml.expval(H)    # Expectation values in the Z basis
